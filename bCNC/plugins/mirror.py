...
class Tool(Plugin):
    ...
    def __init__(self, master):
        ...
        self.variables = [
            ...
            ("pivot_mode", "Document, Selected, Offset", "Document", _("Pivot Mode"), _("Check for doc center, uncheck for collective selection center")),
            ("pivot_ofs", "mm", 0.0, _("Pivot Offset"), _("Mirror pivot offset along selected axis")),
        ]
        self.help = """This plugin mirrors the selected code blocks.

Field descriptions:
===================
- Axis: An axis to mirror around. 
- Pivot Mode:
   - Document: the entire document bounds center is the mirror pivot
   - Selected: selected blocks bounds center is used for mirroring
   - Offset: the mirror pivot is offset by 'Pivot Offset' along the selected 'Axis'
- Pivot Offset: The distance the mirror pivot is offset along selected 'Axis'
"""
    ...
    def execute(self, app):
        ...
        def mod_func(new, old, relative, *args):
            ofs = args[0]
            changed = False
            for axis, C in ofs.items():
                if axis in new:
                    new[axis] = C - new[axis]
                    nonlocal processed_count
                    processed_count += 1
                    changed = True
            if changed and app.gcode.cnc.gcode in (2, 3):  # Change  2<->3
                app.gcode.cnc.gcode = 5 - app.gcode.cnc.gcode
            return changed
        app.gcode.modify(blocks_sel, mod_func, None, mod_args)
        if processed_count > 0:
            app.refresh()
        app.setStatus(_("Mirror: done ({} blocks mirrored around {},{})").format(processed_count, cx, cy))
